...
training, held-out evaluation, and error inspection using the course
classification workflow \cite{course}. Current blocker is sample size: the
median category has only four held-out papers, so one error can materially
change its per-class estimate.
...
optional metadata fields (DOI, journal reference, and comment), title length,
and category tagging, with proportional
...
cross-entropy loss, which accepts unnormalized logits. AdamW used learning rate
0.001 and weight decay 0.0001 \cite{pytorch}. Training used CPU,
...
Training papers fit TF--IDF and network weights. In the artifact set saved July 30,
...
\cite{repository}.
Learning curves come from the metrics artifact saved July 30.

\subsection{Operational research-graph integration}

The classifier now sits beside the repository's daily research knowledge-graph
agent. Its verified August 6 snapshot contains 8,754 documents, 8,951 nodes,
and 59,256 typed edges; every document has at least one edge. GitHub Actions
schedules collection at 13:17 UTC, and the local interface exposes search,
placement explanations, hubs, clusters, and topic trends \cite{repository}.
Graph placement uses recorded arXiv categories, configured source metadata,
and rule-extractor evidence. It does not use classifier predictions as ground
truth or create edges from them. This operational integration expands how the
stored papers can be explored, but it does not change or further validate the
61.37\% held-out classification result reported below.
...
\begin{figure}[H]
  ...
  Right: strict primary-label errors concentrate among several computing
...
epoch 18, training accuracy was 98.44\% ($n=5{,}394$), while validation
accuracy was 62.49\% and validation macro-F1 was 35.34\% (both
$n=1{,}157$); both validation scores declined by epoch 20. Among categories
with at least ten test papers,
...
All nine confusion pairs with at least six errors include Computation and
Language, Artificial Intelligence, Computer Vision and Pattern Recognition,
or Machine Learning on at least one side.
...
cross-list categories, and 61 classes have at most four test papers. The
implementation densifies each 8,000-value TF--IDF row, so memory grows with
papers times features. The split protects held-out rows and duplicate base IDs,
but the same seed-42 test partition was evaluated in an earlier artifact set.
Within the current artifact, validation macro-F1 selected the epoch before test
scoring. This report includes no repeated-seed analysis or confidence intervals.
...
\bibitem{sklearn}
scikit-learn developers, \emph{scikit-learn 1.9 documentation},
\url{https://scikit-learn.org/stable/}, accessed August 5, 2026.

\bibitem{pytorch}
PyTorch contributors, \emph{PyTorch 2.13 documentation},
\url{https://docs.pytorch.org/docs/stable/}, accessed August 5, 2026.

\bibitem{repository}
\emph{paper-fetcher: arXiv metadata, category-classifier code, and executed
evidence notebook}, GitHub repository,
\url{https://github.com/garyzhang1006/paper-fetcher}, accessed August 5, 2026.
